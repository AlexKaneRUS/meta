...
(define (the-mix) '((read program division vs0)
                ...
                (body-pending (ppvs := set-first pending)
                              ...
                              (goto pp-pre-main))
                (pp-pre-main (bb := '((pending := list->set (list (cons (car (car (cdr program))) (make-immutable-hash vs0))))
                                     (static := car division)
                                     (dynamic := cdr division)
                                     (residual := list (filter-vars (car program) static))
                                     (program := make-immutable-hash (cdr program))
                                     (marked := list->set ())
                                     (goto while-pending)))
                             (if (equal? pp 'pre-main) goto next goto pp-while-pending))
                (pp-while-pending (bb := '((if (set-empty? pending) goto exit goto body-pending)))
                                  (if (equal? pp 'while-pending) goto next goto pp-body-pending))
                (pp-body-pending (bb := '((ppvs := set-first pending)
                                          (pending := set-rest pending)
                                          (pp := car ppvs)
                                          (vs := cdr ppvs)
                                          (marked := set-add marked (cons pp vs))
                                          ...
                                          (code := cons (create-label pp vs) ())
                                          (goto while-bb)))
                                  (if (equal? pp 'body-pending) goto next goto pp-end-pending))
                (pp-end-pending (bb := '((residual := append residual (list code))
                                          (goto while-pending)))
                                (if (equal? pp 'end-pending) goto next goto pp-while-bb))
                (pp-while-bb (bb := '((if (null? bb) goto end-pending goto body-bb)))
                             (if (equal? pp 'while-bb) goto next goto pp-body-bb))
                (pp-body-bb (bb := '((command := car bb)
                                     (bb := cdr bb)
                                     (goto case-assign)))
                            (if (equal? pp 'body-bb) goto next goto pp-case-assign))
                (pp-case-assign (bb := '((if (equal? (second command) ':=) goto process-assign goto case-goto)))
                                (if (equal? pp 'case-assign) goto next goto pp-process-assign))
                (pp-process-assign (bb := '((if (is-static (car command) static) goto update-vs goto extend-code-assign)))
                                   (if (equal? pp 'process-assign) goto next goto pp-update-vs))
                (pp-update-vs (bb := '((vs := hash-set vs (car command) (interpret-expr (hash->list vs) (list-to-val (cdr (cdr command)))))
                                       (goto while-bb)))
                              (if (equal? pp 'update-vs) goto next goto pp-extend-code-assign))
                (pp-extend-code-assign (bb := '((code := append code (list (append (list (car command) :=) (reduce (list-to-val (cdr (cdr command))) vs))))
                                                (goto while-bb)))
                                       (if (equal? pp 'extend-code-assign) goto next goto pp-case-goto))
                (pp-case-goto (bb := '((if (equal? (car command) 'goto) goto process-goto goto case-if)))
                              (if (equal? pp 'case-goto) goto next goto pp-process-goto))
                (pp-process-goto (bb := '((bb := hash-ref program (cadr command) ())
                                          (goto while-bb)))
                                 (if (equal? pp 'process-goto) goto next goto pp-case-if))
                (pp-case-if (bb := '((if (equal? (car command) 'if) goto process-if goto case-return)))
                            (if (equal? pp 'case-if) goto next goto pp-process-if))
                (pp-process-if (bb := '((if (is-static (second command) static) goto static-conditional goto dynamic-conditional)))
                               (if (equal? pp 'process-if) goto next goto pp-static-conditional))
                (pp-static-conditional (bb := '((pp-quote := hash-ref program (fourth command) ())
                                                (pp-quote-quote := hash-ref program (sixth command) ())
                                                (if (interpret-expr (hash->list vs) (second command)) goto static-pp-quote goto static-pp-quote-quote)))
                                       (if (equal? pp 'static-conditional) goto next goto pp-static-pp-quote))
                (pp-static-pp-quote (bb := '((bb := pp-quote)
                                             (goto while-bb)))
                                    (if (equal? pp 'static-pp-quote) goto next goto pp-static-pp-quote-quote))
                (pp-static-pp-quote-quote (bb := '((bb := pp-quote-quote)
                                                   (goto while-bb)))
                                    (if (equal? pp 'static-pp-quote-quote) goto next goto pp-dynamic-conditional))
                (pp-dynamic-conditional (bb := '((pending := set-subtract
                                                          (set-union pending (list->set (list (cons (fourth command) vs) (cons (sixth command) vs))))
                                                          marked)
                                                 (code := append code (list (append
                                                                       (append (list if) (list (reduce (second command) vs)))
                                                 ...
                                                 (goto while-bb)))
                                    (if (equal? pp 'dynamic-conditional) goto next goto pp-case-return))
                (pp-case-return (bb := '((if (equal? (car command) 'return) goto process-return goto process-error)))
                                (if (equal? pp 'case-return) goto next goto pp-process-return))
                (pp-process-return (bb := '((code := append code (list (append (list return) (reduce (cdr command) vs))))
                                            (goto while-bb)))
                                (if (equal? pp 'process-return) goto next goto pp-process-error))
                (pp-process-error (bb := '((return error "Mix: unknown expression")))
                                  (if (equal? pp 'process-error) goto next goto pp-exit))
                (pp-exit (bb := '((exit (return residual))))
                         (if (equal? pp 'exit) goto next goto process-error))
                (next (if (equal? next-c 'while-bb) goto while-bb goto body-pending1))
...
(define (second-futamura) (pretty-print (interpret (mix) (list
                                             (the-mix)
                                             (cons '(program division static dynamic bb next-c command pp-quote pp-quote-quote) '(pending marked residual ppvs pp vs vs0 code))
                                             (list (cons 'program (tm-interpreter))
                                                   (cons 'division (cons '(Q Qtail Instruction Operator Symbol NextLabel) '(Right Left)))))))) 